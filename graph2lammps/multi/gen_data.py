import numpy as np
import itertools
from graph2lammps.convert import read_pdb
...
def uniform_sample(vmin, vmax, n):
    ...
    samples = [np.linspace(vmin[i], vmax[i], int(ranges[i]), endpoint=False) for i in range(len(vmin))]
    
    indices = np.random.choice(total_points, n, replace=False)
    
    points = np.array([samples[i][indices % int(ranges[i])] for i in range(len(vmin))]).T
...
def gen_data(pos, args):
    if args.pdb != None:
        ...
        npoly = L ** 3 / ((1-args.rho) / args.rho) / nbead # number of polymer chains 2 wt%
        npoly = int(npoly + 0.5)
        pos_out = read_pdb(args.pdb)
        vmax  = np.max(np.abs(pos_out))
        pos_out = pos_out * (L / 2 / vmax)
